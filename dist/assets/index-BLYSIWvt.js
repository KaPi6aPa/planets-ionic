...
const Preferences = registerPlugin("Preferences", {
  web: () => __vitePreload(() => import("./web-YLYzXWr8.js"), true ? [] : void 0).then((m) => new m.PreferencesWeb())
});
const API_URL = "https://university-api-alpha.vercel.app/api/planets";
const CUSTOM_PLANETS_KEY$1 = "custom_planets";
async function loadCustomPlanets$1() {
  try {
    const { value } = await Preferences.get({ key: CUSTOM_PLANETS_KEY$1 });
    if (!value) return [];
...
class HomePage extends HTMLElement {
  ...
  async init() {
    this.customPlanets = await loadCustomPlanets$1();
    this.render();
  ...
  async fetchPlanetsData() {
    ...
    try {
      ...
      this.errorMessage = "Не вдалося завантажити дані з сервера. Спробуйте оновити сторінку пізніше.";
      this.apiPlanets = [];
    } finally {
  ...
  getAllPlanets() {
    ...
    if (this.currentSort === "name-asc") {
      ...
      all.sort((a, b) => {
        var _a, _b;
        const ma = parseFloat((((_a = a.details) == null ? void 0 : _a.mass) || "").replace(/[^\d.]/g, "")) || 0;
        const mb = parseFloat((((_b = b.details) == null ? void 0 : _b.mass) || "").replace(/[^\d.]/g, "")) || 0;
        return ma - mb;
      });
    }
    return all;
  ...
  render() {
    ...
    const errorHtml = this.errorMessage ? `<div class="error-banner">${this.errorMessage}</div>` : "";
    const planetsHtml = planets.length === 0 ? `<p class="ion-padding">Немає даних про планети.</p>` : planets.map(
      (planet) => `
...
class PlanetPage extends HTMLElement {
  ...
  async init() {
    ...
    const custom = await loadCustomPlanets$1();
    ...
    const satellitesBlock = d.satellites ? `
        <ion-accordion value="satellites">
          ...
          <div class="ion-padding" slot="content">
            <p>${Array.isArray(d.satellites) ? d.satellites.join(", ") : d.satellites}</p>
          </div>
        </ion-accordion>
      ` : "";
    const missionsBlock = d.missions ? `
        <ion-accordion value="missions">
          ...
          <div class="ion-padding" slot="content">
            <ion-list>
              ${Array.isArray(d.missions) ? d.missions.map((m) => `<ion-item>${m}</ion-item>`).join("") : `<ion-item>${d.missions}</ion-item>`}
            </ion-list>
          </div>
        </ion-accordion>
      ` : "";
    ...
    confirmAddPlanetButton.addEventListener("click", async () => {
      ...
      const newPlanet = {
        ...
        details: {
          ...
          satellites: satellitesInput.value.split(",").map((s) => s.trim()).filter((s) => s),
          missions: missionsInput.value.split(",").map((m) => m.trim()).filter((m) => m)
        }
      };
